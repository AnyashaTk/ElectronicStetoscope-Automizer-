...
- Product Owner для данного продукта - лабоработии центра Альбрехта. В качестве бизнес метрик будет измеряться медицинская основная метрика - эффективность улучшения качества жизни пациентов
...
- Острота слуха, индивидуальная интерпретация разных шумов также осложняют работу терапевтов и кардиологов при аускультации сердца
...
- Алгоритм должен принимать на вход аудиофайл с записью с электростетоскопа, на выходе возвращать либо значения 1\0, либо в лучшем случае вектор со значениями 1\0 для каждой патологии и шумов
- Проект может иметь возможность работать локально, без использования гпу, так и на сайте с использованием крупных моделей при надобности
...
- Отношение сигнал/шум (SNR) после очистки данных. Целевое значение: SNR > 20 дБ, удаление речи > 95%
...
- В первую итерацию входит EDA данных, полученных от Product Owner, анализ открытых датасетов, планирование экспериментов по предобработке данных и классификации наличия патологий
- По итогам итерации клиенту будет передан репозиторий с сервисом для поднятия одностраничного сайта для обработки аудиофайлов, полученных с помощью электронного стетоскопа, и при получении, веса модели. Алгоритмы обучения модели клиенту не требуются на данный момент
- Дальнейшие разработки включают в себя улучшение качества предобработки данных и классификации, поднятие метрик, расширение количества или видов патологий. Помощь со встраиванием алгоритма в новое устройство, на данный момент не входят в скоуп задач  
...
- будет использована обученная на данных модель для классификации шумов
- на основе полученных результатов будет подбираться текстовая рекомендация для лечащего врача

Риски данного этапа:
- на данный момент есть трудности с поиском открытых данных, несмотря на количество статей о таковых. 
...
Полностью оконченным этап считается при получении обученной модели бинарной классификации, с метриками валидации не сильно уступающим указанным в пункте 1.2, и при возможности поднятия одностраничного сайта для использования модели\заглушки

#### 3.3 Подготовка MVP 
Прототип (версия до MVP) написана на streamlit и включает в себя возможность опробовать работу проекта с семплами здорового и диагностированного пациента. 


### 4 Внедрение\продакшн

#### 4.1 Архитектура решения
Архитектура решения на данный момент на 80% совпадает с предполагаемой архитектурой бейзлайна
...
Особых требований к системе на данный момент не требуется, тк не планируется использование крупных моледей либо же большого потока пользователей
